...
class LoginScreen extends StatelessWidget {
  ...
  Future<String> _authUser(LoginData data) {
    return Future.delayed(loginTime).then((_) async {
      ...
      var response = await db.login(data.name, encrypter.encrypt(data.password, iv: iv).base64);
      if (response == "invalid") {
  ...
  Future<String> _registerUser(LoginData data) {
    return Future.delayed(loginTime).then((_) async {
      ...
      UserModel userModel = new UserModel(data.name, encrypter.encrypt(data.password, iv: iv).base64);
